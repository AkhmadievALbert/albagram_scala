...
import infrastructure.endpoint.comment.CommentEndpoints
...
import infrastructure.repository.{DoobieAuthRepositoryInterpreter, DoobieCommentRepositoryInterpreter, DoobiePostRepositoryInterpreter, DoobieThemeRepositoryInterpreter, DoobieUserRepositoryInterpreter}
...
object Server extends IOApp {
  def createServer[F[_] : ContextShift : ConcurrentEffect : Timer]: Resource[F, H4Server[F]] =
    for {
      ...
      commentService = CommentService(commentRepo, postValidation)
      ...
      httpApp = Router(
        ...
        "/comments" -> CommentEndpoints
          .endpoints[F, HMACSHA256](commentService, routeAuth)
